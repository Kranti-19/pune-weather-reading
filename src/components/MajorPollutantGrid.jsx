  ...
  switch (key) {
    ...
    case "NH3":
      return {
        formula: (
          <>
            NH<sub className="font-bold">3</sub>
          </>
        ),
        fullName: "Ammonia",
      };
    ...
    <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-4 sm:p-5 font-sans">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-2.5">
        ...
        {pollutants.map((pollutant, index) => {
          ...
          const theme = getRibbonTheme(
            pollutant?.value,
            pollutant?.standard
          );

          const display = getPollutantDisplay(
            pollutant?.name
          );
          ...
          /*
           * IMPORTANT:
           * Use a guaranteed unique React key.
           *
           * Previously:
           * key={pollutant.name || pollutant.key}
           *
           * This can produce duplicate keys if two pollutants
           * have the same name.
           *
           * Now we combine key/name/index.
           */
          const uniqueKey = `${
            pollutant?.key ||
            pollutant?.name ||
            "pollutant"
          }-${pollutant?.name || "unknown"}-${index}`;
            ...
            >
              ...
              <div className="mt-3 pt-2 border-t border-slate-100 flex items-center justify-between gap-1">
                ...
                >
                  ...
                  {theme.statusText === "Within Safe Limit"
                    ? "Safe"
                    : theme.statusText === "Moderate Exposure"
                    ? "Moderate"
                    : theme.statusText === "No Data"
                    ? "No Data"
                    : "Exceeded"}
