...
const normalizeStation = (station, index) => {
  ...
  return {
    ...
    station_type: station.station_type ?? station.stationType ?? "CAAQM",
    aqi: Number(aqi) || 65,
    ...
    status,
    updated,
    lastReadingAt: station.lastReadingAt ?? rawUpdated,
    deviceCount: Number(station.deviceCount) || 0,
  };
};
...
export default function PuneAreas() {
  ...
  const loadStationsFromAPI = useCallback(async () => {
    ...
    try {
      ...
      if (list.length > 0) {
        ...
        setStations([]);
        setError("No monitoring stations were found in the database.");
      }
    } catch (err) {
      console.error("Error loading stations from backend:", err);
      setStations([]);
      setError(
  ...
  const filteredStations = useMemo(() => {
    ...
    return stations.filter((station) => {
      ...
      const ward = String(station?.ward ?? "").toLowerCase();
      const id = String(station?.id ?? "").toLowerCase();
      const zone = String(station?.zone ?? "").toLowerCase();
  ...
  const attentionRequired = stations.filter((station) => {
    const aqi = Number(station?.aqi) || 65;
    return station?.status === "Offline" || aqi > 200;
  }).length;
  ...
  const handleStationNavigation = (station) => {
    const safeId =
      station?.numericId ||
      station?.station_id ||
      (station?.id ? String(station.id).replace(/\D/g, "") : "");

    if (!safeId) {
      console.error("Station ID missing:", station);
      return;
    }

    navigate(`/station/${safeId}`);
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-7">
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          ...
          >
            <RefreshCw
              size={13}
              className={loading ? "animate-spin text-blue-600" : "text-slate-400"}
            />
            <span>{loading ? "Loading..." : "Refresh Stations"}</span>
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-[0_8px_25px_rgba(15,23,42,0.05)] mb-7">
        ...
        {loading ? (
          <div className="py-16 flex flex-col items-center justify-center text-slate-400">
            ...
            <p className="text-sm font-semibold">Loading stations from database...</p>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredStations.length === 0 ? (
                  ...
                  filteredStations.map((station, index) => {
                    ...
                    const stationId =
                      station?.id ||
                      `PMC-${String(index + 1).padStart(3, "0")}`;
                    ...
                    return (
                      <tr
                        ...
                        onClick={() => handleStationNavigation(station)}
                      >
                        ...
                        <td className="py-4">
                          <span className="inline-flex items-center gap-1.5 text-xs font-bold px-2.5 py-1 bg-slate-100 text-slate-700 rounded-xl border border-slate-200/60">
                            ...
                            <span>{station?.dominant || "pm10"}</span>
                          </span>
                        </td>
                        ...
                        <td className="py-4 text-right pr-2">
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              handleStationNavigation(station);
                            }}
